# 📦 Datos de Ejemplo para el Sistema

Si quieres probar el sistema con datos de ejemplo, aquí tienes algunas sugerencias de productos que puedes agregar manualmente desde el panel de administración.

## 💻 Computadoras

### Laptop Dell Latitude 7420
- **Categoría:** Computadoras
- **Marca:** Dell
- **Modelo:** Latitude 7420
- **Número de Serie:** DLLAT7420-001
- **Estado:** Disponible
- **Ubicación:** Almacén Principal
- **Fecha de Adquisición:** 2024-01-15
- **Precio:** 1200.00
- **Descripción:** Laptop empresarial Intel Core i7, 16GB RAM, 512GB SSD

### Desktop HP ProDesk 600 G6
- **Categoría:** Computadoras
- **Marca:** HP
- **Modelo:** ProDesk 600 G6
- **Número de Serie:** HPDESK600-002
- **Estado:** En Uso
- **Ubicación:** Oficina 201
- **Fecha de Adquisición:** 2023-08-20
- **Precio:** 850.00
- **Descripción:** PC de escritorio Intel Core i5, 8GB RAM, 256GB SSD

### Laptop Lenovo ThinkPad X1 Carbon
- **Categoría:** Computadoras
- **Marca:** Lenovo
- **Modelo:** ThinkPad X1 Carbon Gen 9
- **Número de Serie:** LNVX1C-003
- **Estado:** Disponible
- **Ubicación:** Almacén Principal
- **Fecha de Adquisición:** 2024-03-10
- **Precio:** 1450.00
- **Descripción:** Ultrabook empresarial Intel Core i7, 16GB RAM, 1TB SSD

---

## 🖨️ Periféricos

### Monitor Dell UltraSharp 27"
- **Categoría:** Periféricos
- **Marca:** Dell
- **Modelo:** U2720Q
- **Número de Serie:** DLMON27-004
- **Estado:** En Uso
- **Ubicación:** Oficina 205
- **Fecha de Adquisición:** 2024-02-05
- **Precio:** 450.00
- **Descripción:** Monitor 4K IPS 27 pulgadas

### Teclado Mecánico Logitech MX Keys
- **Categoría:** Periféricos
- **Marca:** Logitech
- **Modelo:** MX Keys
- **Número de Serie:** LGMXK-005
- **Estado:** Disponible
- **Ubicación:** Almacén Principal
- **Fecha de Adquisición:** 2024-04-12
- **Precio:** 120.00
- **Descripción:** Teclado mecánico inalámbrico retroiluminado

### Impresora HP LaserJet Pro
- **Categoría:** Periféricos
- **Marca:** HP
- **Modelo:** LaserJet Pro MFP M428fdw
- **Número de Serie:** HPLJ428-006
- **Estado:** En Uso
- **Ubicación:** Oficina 202
- **Fecha de Adquisición:** 2023-11-18
- **Precio:** 550.00
- **Descripción:** Impresora láser multifunción monocromática

---

## 🌐 Redes

### Router Cisco RV340
- **Categoría:** Redes
- **Marca:** Cisco
- **Modelo:** RV340
- **Número de Serie:** CSCRV340-007
- **Estado:** En Uso
- **Ubicación:** Sala de Servidores
- **Fecha de Adquisición:** 2023-06-25
- **Precio:** 380.00
- **Descripción:** Router VPN Dual WAN Gigabit

### Switch TP-Link 24 Puertos
- **Categoría:** Redes
- **Marca:** TP-Link
- **Modelo:** TL-SG1024D
- **Número de Serie:** TPSG24-008
- **Estado:** En Uso
- **Ubicación:** Sala de Servidores
- **Fecha de Adquisición:** 2023-07-10
- **Precio:** 150.00
- **Descripción:** Switch Gigabit no administrable 24 puertos

### Access Point Ubiquiti UniFi
- **Categoría:** Redes
- **Marca:** Ubiquiti
- **Modelo:** UniFi AP AC Pro
- **Número de Serie:** UBIAP-009
- **Estado:** Disponible
- **Ubicación:** Almacén Principal
- **Fecha de Adquisición:** 2024-01-30
- **Precio:** 180.00
- **Descripción:** Access Point WiFi AC Dual Band

---

## 🗄️ Servidores

### Servidor Dell PowerEdge R450
- **Categoría:** Servidores
- **Marca:** Dell
- **Modelo:** PowerEdge R450
- **Número de Serie:** DLSRV450-010
- **Estado:** En Uso
- **Ubicación:** Sala de Servidores
- **Fecha de Adquisición:** 2023-05-15
- **Precio:** 3500.00
- **Descripción:** Servidor rack Intel Xeon, 32GB RAM, 2x 1TB HDD

### Servidor HP ProLiant DL380 Gen10
- **Categoría:** Servidores
- **Marca:** HP
- **Modelo:** ProLiant DL380 Gen10
- **Número de Serie:** HPSRV380-011
- **Estado:** En Reparación
- **Ubicación:** Sala de Servidores
- **Fecha de Adquisición:** 2022-12-08
- **Precio:** 4200.00
- **Descripción:** Servidor rack Intel Xeon, 64GB RAM, 4x 2TB HDD RAID5

---

## 💾 Almacenamiento

### NAS Synology DS920+
- **Categoría:** Almacenamiento
- **Marca:** Synology
- **Modelo:** DiskStation DS920+
- **Número de Serie:** SYNNAS920-012
- **Estado:** En Uso
- **Ubicación:** Sala de Servidores
- **Fecha de Adquisición:** 2023-09-22
- **Precio:** 650.00
- **Descripción:** NAS 4 bahías con 4x 4TB HDD

### Disco Duro Externo WD 4TB
- **Categoría:** Almacenamiento
- **Marca:** Western Digital
- **Modelo:** My Passport 4TB
- **Número de Serie:** WDEXT4TB-013
- **Estado:** Disponible
- **Ubicación:** Almacén Principal
- **Fecha de Adquisición:** 2024-05-05
- **Precio:** 120.00
- **Descripción:** Disco duro externo portátil USB 3.0

---

## 📝 Cómo agregar estos datos

1. Inicia sesión en el sistema
2. Ve a **"Productos"** → **"Nuevo Producto"**
3. Copia la información de arriba
4. Llena el formulario
5. **(OPCIONAL)** Agrega una imagen del producto (PNG, JPEG o WEBP)
6. Guarda
7. Repite para cada producto que quieras agregar

## 📷 Sobre las Imágenes

- **Opcional:** No es obligatorio agregar imágenes
- **Dos opciones:**
  1. **📤 Subir archivo:** PNG, JPEG, WEBP (máx. 5MB)
  2. **🔗 URL externa:** Pega link de imagen del fabricante
- **Recomendación:** Usa imágenes cuadradas para mejor visualización
- **Dónde conseguir:**
  - Fotos reales de tus equipos (subir archivo)
  - URLs del sitio del fabricante (ej: dell.com, hp.com)
  - Bancos de imágenes gratuitas (Unsplash, Pexels)

## 💡 Consejos

- Puedes modificar los precios y fechas según necesites
- Los números de serie deben ser únicos
- Ajusta las ubicaciones según tu organización
- Modifica los estados según la situación real de cada equipo

---

**¡Estos son solo ejemplos! Puedes agregar tus propios equipos con la información real de tu organización.**

